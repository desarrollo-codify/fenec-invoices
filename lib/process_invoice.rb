class ProcessInvoice
  ...
  def self.process(invoice, point_of_sale, economic_activity)
  ...
  def self.sent_to_siat(invoice)
    SendInvoiceJob.perform_later(invoice)
  end

  def self.current_daily_code(branch_office, point_of_sale)
    branch_office.daily_codes.where(point_of_sale: point_of_sale.code).current
  end

  def self.process_cafc(invoice, economic_activity, point_of_sale)
  ...
  def self.daily_code(invoice, point_of_sale)
  ...
  def self.process_pending_data(invoice, point_of_sale, siat_available, economic_activity)
  ...
  def self.cuf(invoice_date, current_number, control_code, point_of_sale)
  ...
  def self.invoice_number(point_of_sale)
    branch_office = point_of_sale.branch_office
  ...
  def self.module_eleven(code, limit)
    sum = 0
    multiplier = 2
  ...
  def self.hex_base(value)
    value.to_s(16)
  end

  def self.qr_content(nit, cuf, number, page_size)
